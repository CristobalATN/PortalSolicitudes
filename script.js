...
function enviarDatosAPowerAutomate(datos, tipoSolicitud) {
    ...
    async function guardarEnFirestore() {
        try {
            ...
            const { getFirestore, collection, addDoc, serverTimestamp } = await import("https://www.gstatic.com/firebasejs/10.9.0/firebase-firestore.js");
            ...
            await addDoc(collection(db, 'solicitudes'), {
                ...
                email:         datosCompletos.emailValidacion || '',
                datos:         datosCompletos,
                estado:        'recibida',
    ...
    // Enviar a Power Automate Y guardar en Firestore en paralelo
    Promise.all([
    ...
    .then(async ([response]) => {
        ...
        if (!response.ok) {
            ...
            if (errorText) console.error('Error response body:', errorText);
        ...
        if (window.parent !== window) {
            window.parent.postMessage('solicitud-enviada', '*');
        } else {
